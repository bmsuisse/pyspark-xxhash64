...
once, computed natively in Rust (see ``rust/crates/spark-xxhash64-arrow``)
instead of one Python value at a time via :mod:`pyspark_xxhash64.hasher`.

Arrow-implementation-agnostic, not pyarrow-specific: both directions go
through the standardized `Arrow PyCapsule Interface
...
Requires the optional ``spark-xxhash64-arrow`` wheel (built with maturin
...
try:
    import spark_xxhash64_arrow as _native
...
def _require_native() -> Any:
    if _native is None:
        raise ImportError(
            "pyspark_xxhash64.arrow requires the compiled 'spark-xxhash64-arrow' "
            "extension. Build it with `maturin develop` (or `pip install`) from "
            "rust/crates/spark-xxhash64-arrow, see that folder's README."
        ) from _import_error
    return _native
